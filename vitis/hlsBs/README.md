...
**CAUTION:** 2023.2 being the first version to support the unified development environment, has some *pecularities*.  **hlsBs** has workarounds, but as with all workarounds, there are pitfalls.
...
- Make the commands as efficient/fast as possible
	...
	- Even for steps where the Vitis startup overhead is small compared to the execution time such as synthesis, **hlsBs** sometimes can detect errors before invoking the costily Vitis utility.
...
- Similar command structure
	- The most commonly used commands uniformily target component(s) by name and catagory.
...
In keeping with the goal of being able to mix techniques, it is permissiable for the configuration files, the workspace and components to be generated by some other means. The most common example of this would be legacy projects. However, not using **hlsBs** to generate these products will limit some of its most useful features.
...
| hlsComp   | Creates, replaces, cleans, lists the components. Useful when only have the configuration file. - *rarely used* |
...
1. As indicated before, target names may include wildcards, which the shell is eager to translate by finding matching patterns in the file names of the current working directory. Of course, unless the current working directory is the target of the translation, this is will not yield the desired results.
...
- Python is familar to many and is a good match for the task
- Python can implement the simple to the complex
- Errors are nicely reported by the Python interpretter
...
| Variable           | Meaning |
| :----------------- | :------ |
| HLSBS_XILINX_SETUP | The search paths for the Xilinx/Vitis setttings scripts |
| HLSBS_PROJECT      | The project file |
| HLSBS_WORKSPACE    | The workspace |
| HLSBS_PRODUCTS     | The products directory |
| HLSLBS_INI         | This is a colon separated list of @FILES |
...
**HLSBS_INI** allows one to collect a number indirect files used to modify command line parameters. This is not ideal. It is unfortunate that argparse does not allow an include mechanism so that files with well-defined purposes can be mixed and matched.
...
Here *'{vitis_version}'* is a logical symbol.  This allows workspace to be distinguished by the Vitis version and wil be resolved when the workspace is referenced.<br>

The workspace can be overidden from the command line either with an explicit command line parameter or the **HLSBS_WORKSPACE** environment variable.  This is useful for creating ad hoc, experimental versions of the workspace.  A common tactic is to relocate the workspace to a throw-away directory in /tmp/.
...
``` python
    build  = ( 'build', { 'top'       : 'doit',
                          ...
                          'cosim_argv' : '--ntests=100'} )
```
There can be a single build defintion (as in this example) or list/tuple of builds giving the name of the build product and a dictionary defining its makeup. The name (the *'build'*) is often used when composing the configuration file path and component name. In more advanced usages, this could be put in Python loop to compose variations. An example where multiple builds would be useful is if a project passes data through multiple FPGAs, each performing different processing on the data.
...
**NOTE:** Even though '*tb' and *'syn'* can accept a list or tuple, they are not specified as plurals on (debatable) idea the testbench and synthesis are singular entities that *may* be composed of more than one thing.  There are not multiple testbenches and hls synthesis'es.
...
The *id* can, as can the *part*, *clock* and *fpga_uncertainity*, be used as logical symbols to generate a unique component name.  Here the *clock* could serve the same purpose since it is different. The *fpga_id* serves as convenient user chosen nickname for the FPGA. It could convey, for example, both a different FPGA part and clock.
...
| :-----  | :-------- |
| build   | build_id  |
| fpga    | fpga_part <br>fpga_clock<br>fpga_uncertainity<br>fpga_id |

**RECOMMENDATION**:  The names *'build'* and *'fpga'*, while meaningful, are user chosen. In more complex buids, these prefixes may need to be different to avoid name clashes.<br> To be safe, limit to the characters used to form legitimate Python variables.<br>
...
> A nuance should be noted here. While the component name must be unique, only configuration path name, not the configuration file name, needs to be since, while not recommended, the configuration file could be placed in its own distinguishing directory.  If this is done, *hlsBs* supplies the logical *cfg_dir* for the first directory to help in generating a unique component name. Note that only the first subdirectory is provided. There is currently no prescribed method if this is sitll not sufficient to generate a unique component name from the configuration path; it would have to be constructed independently.

**NOTE**: The configuration file could just have easily been named after the component. Naming the component after the configuration file was just a matter of choice in this example.
...
- Even though there is only one target, this has been specified as a list to emphasis it can be a list or tuple. It can also be just the single element.
...
These are modified Vitis products produced by **hlsBs**.  They have proven to be useful in overcoming some of the limitations of the Vitis products they are derived from. There are 2 such products, DCP renaming and expanding the permissiable FPGA target families.

#### DCP Renaming
All standard Vitis/Vivado produced *.dcp* file have the same embedded name. This makes it impossible to combine multiple *.dcp* files when producing a single FPGA image.  This allows unique names to be assigned to the *.dcp* files.

#### FPGA Family Augmentation
Using a Vivado utility, the *.zip* file is repackaged to include a specified list of allowable FPGAs families.
...
All the values have sensible defaulted, except for the *'family'*.  While it does have a default value, it almost certainly is not what is wanted. The values above are the default values. They can all be omitted if the defaults are acceptable.
...
> Even worse, if the configuration is built not using **hlsBs**, but in the IDE/GUI, the value of the environment variables are whatever they were when launched.  If one assumes the values are when the configuration file was made (possibly 2 weeks ago), a sad ending may await.  This is not a theortical problem. It cost the author a couple of days to learn this.  It is one reason why **hlsBs** is very careful in its handling of environmental variables. **hlsBs** does have the advantage that all its commands inherit the context of the shell they are launched in, so any enviroment variables have the values of that shell.  This is not true of the IDE/GUI. The values of the environment variables are whatever there were at the time it was launched from whatever shell.

> Even more worse, if the execution is by prechance in a directory containing the test files where the fact that the environment variable translates to nothing and they are *accidently* found by the relative path, it might succeed, but tomorrow, when in a different directory, it might mysteriously fail.
...
This is personal preference, but do be sure to think about which products should be commited to GIT, else some very large files could be committed to your GIT repository.

**RECOMMENDATIONS**:
- Add the workspace directory to the .gitignore.
	- The ambitious may wish to explore saving all but the hls/ and logs directory
    - This would preserve the component definition, saving recreating it on checkout and the reports, which may be used for tracking purposes.
    - If there is interest, a option to automatically add these to the appropriate gitignore could be added to **hlsBs**.
- Whether to commit the configuration directory and files to the git repository is a user choice.
	...
	- If they are committed, their corresponding components, which are stored in the workspace are generally not, so the components must be recreated on checkout using **hlsComp**.  This diminishes the usefulnesss of committing the configuration files if they can easily be recreated by **hlsCfg**.
...
- Expand the number of parameters available on the command line.
	- In developing HLS code, the ability to do ad hoc experimentation and then capture and catalog the useful ones is invaluable. Being able to experiment on the command line goes a long ways in realizing this.
