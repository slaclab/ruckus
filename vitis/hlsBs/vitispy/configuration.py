...
class Configuration :
    ...
    def __init__ (self, args, create_components, project, product) :
        ...
        if configurations : configurations = os.path.expandvars (configurations)
    ...
    def execute  (self, target) :
        ...
        if  hasattr (build, 'sim') :
            ...
            if   sim_argv : self.sim_argv = sim_argv.format_map (target.map)
            else          : self.sim_argv = None
        else :
            self.sim_argv = None

        if  build.csim_argv :
            csim_argv = build.csim_argv
            if   csim_argv : self.csim_argv = csim_argv.format_map  (target.map)
        ...
        if (self.comp)  and not self.dry_run:
            status, ignore, msg = self.comp.execute (self.client,
                                                     target.cmp_name,
                                                     target.cfg_path)
            if (exists) : return True, ["Replaced", msg]
            else        : return True, ["Created",  msg]

        elif self.comp and self.dry_run :
            self.comp.cmp_dir = os.path.join (self.workspace, target.cmp_name)

        if (exists) : return True, ["Replaced"]
        else        : return True, ["Created"]
    ...
    def compose_path (cfg_file) :
        ...
        if (extension) : return cfg_file
        else           : return cfg_file + '.cfg'
    # --------------------------------------------------------------------------
    ...
    def include_add (need_import, import_file, rel_path, dname, file) :
        defs = ''
        if need_import :
            import_file = os.path.relpath (os.path.realpath (import_file), rel_path)
