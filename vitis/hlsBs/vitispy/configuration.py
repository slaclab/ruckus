...
class Configuration:
    ...
    def expand_defs(defines,
        ...
        for d in defines:
            ...
            # ---------------------------------------
                ...
                # Route through the generated defines header (see _add_files).
                # csim cflags take one shell pass while cosim re-tokenizes them
                # verbatim, so no single quoted -D value survives both; emit
                # `#define NAME "path"` in a header to keep the argv quote-free.
                hdrs.append((name, file))
            ...
            # -------------------------------------------------
                ...
                # Route through the generated defines header (see _add_files).
                # csim cflags take one shell pass while cosim re-tokenizes them
                # verbatim, so no single quoted/bare -D value survives both;
                # emitting `#define NAME "path"` in a header keeps the argv
                # quote-free and lets the testbench use a plain `#include MACRO`.
                hdrs.append((name, rel_file))
        ...
        return errs, defs, hdrs
    # --------------------------------------------------------------------------

    # --------------------------------------------------------------------------
    # Emit file-path defines as `#define NAME "path"` lines in a small
    # generated header and return the cflags that force it in (-include) plus
    # its search dir (-I). The quotes live in the header, never on the command
    # line: csim takes one shell pass over the cflags while cosim re-tokenizes
    # them verbatim, so a quoted -D can never satisfy both, but -include/-I are
    # quote-free and reach each compiler identically. The header is referenced
    # by its bare name via -include and located through -I: Vitis compiles the
    # sources from a per-component sub-directory (not the cfg directory) and
    # rewrites -I paths to absolute, so a bare -include is found on the -I
    # search chain the same way the testbench's own `#include "..."` are. A
    # directory-qualified -include would instead be looked up under each -I and
    # miss (doubled directory), which is why only the base name is emitted.
    # --------------------------------------------------------------------------

    @staticmethod
    def _write_defs_header(cfg_path,
                           cfg_name,
                           label,
                           idx,
                           hdrs):

        defs_dir = os.path.join(cfg_path, 'hlsBs_defs')
        os.makedirs(defs_dir, exist_ok=True)

        hdr_name = f'{cfg_name}_{label}{idx}_defs.h'
        hdr_path = os.path.join(defs_dir, hdr_name)

        with open(hdr_path, 'w') as hdr:
            hdr.write('// Generated by hlsBs -- do not edit.\n')
            for name, path in hdrs:
                hdr.write(f'#define {name} "{path}"\n')

        return ' -include ' + hdr_name + ' -I hlsBs_defs'
    # --------------------------------------------------------------------------
    ...
    def _add_files(cfg_file,
        ...
        for idx, file in enumerate(files):
            ...
            # ------------------------------------------------
            if 'defines' in file:
                errs, defs, hdr_pairs = Configuration.expand_defs(
                    file['defines'],
                    cfg_name,
                    file_path,
                    map)
            ...
            if incs or defs or hdr_pairs:
                ...
                if defs:
                    cflags += defs
                if hdr_pairs:
                    cflags += Configuration._write_defs_header(cfg_path,
                                                               cfg_name,
                                                               label,
                                                               idx,
                                                               hdr_pairs)
