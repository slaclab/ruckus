...
class Lister (DoAction) :
    ...
    def list (self, idx, cfg_path, cmp_path, seps) :
        ...
        exists   = (cfg_path != None and os.path.isfile (cfg_path))
        sep      = seps[0] if exists else  seps[1]

        if not cfg_path : cfg_path = "None Found"
...
class Cleaner (DoAction) :
    ...
    def clean (self, idx, cfg_path, cmp_path, seps) :
        ...
        if cfg_path : cfg_exists = os.path.isfile (cfg_path)
        else        :
            cfg_exists = False
            cfg_path   = "None"
...
def merge (xl, yl) :
    ...
    if not xl and not yl : return None
    xy = ''
    if xl :
        # Insure xl is a list
...
def main () :
    ...
    # ---------------------------------------------------------
    if   args.list     is not None : op = "Listing"
    elif args.clean    is not None : op = "Cleaning"
    elif args.replace  is not None : op = "Replacing"
    else                           : op = "Creating"
    ...
    # ---------------------------------------
        ...
        if   replace :
            ...
            if args.create is None : args.create = []
