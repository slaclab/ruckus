...
class Target:
    '''
    A target is a candidate for an eventual build of an HLS configuration file and
    ...
    def __init__(self,
        ...
        Args:
           workspace   :  The fully resolved path to the VITIS/HLS  workspace
        ...
        self.fpga = map_list[ctb_types[1]].object


        # ---------------------------------------------------
        # Complete the configuration and component file paths
        # ---------------------------------------------------
        cfg_tmp = os.path.expandvars (cfg_template.template)
        cfg_pre = cfg_template.prefix
        cmp_tmp = os.path.expandvars (cmp_template.template)
        cmp_pre = cmp_template.prefix

        cfg_root = cfg_root.format_map (map)

        self.complete_cmp_path (workspace, map, cmp_pre, cmp_tmp)
        self.complete_cfg_path (cfg_root,  map, cfg_pre, cfg_tmp)

        # -----------------------------------------------------
        # Could have failed to complete the cmp_path because it
        # depended on the cfg_path name which was not available
        # until the cfg_path was completed.
        # -----------------------------------------------------
        ...
        if self.cmp_path is None or self.cfg_path is None :
            msg = ""
            cmp = f"     : Product.CmpTemplate -> prefix = '{cmp_pre}', template = '{cmp_tmp}'\n"
            cfg = f"     : Product.CfgTemplate -> prefix = '{cfg_pre}', template = '{cfg_tmp}'\n"

            if self.cmp_path is None and self.cfg_path is None:
                msg = ("Neither the component nor configuration path coud be resolved, check\n" +
                        cmp + cfg)

            elif self.cmp_path is None:
                msg =  "The component path could not be resolved, check\n" + cmp

            elif self.cfg_path is None:
                msg =  "The configuration path could not be resolved, check\n" + cfg
        ...
        if tgt_key:
            ...
            self.tgt_name = os.path.split(
                os.path.splitext(self.tgt_path)[0])[1]
        else:
            self.tgt_key = None
            self.tgt_path = None
            self.tgt_name = None
    ...
    # --------------------------------------------------------------------------
    def complete_cmp_path (self, workspace, map, prefix, template):
        try:
            self.cmp_path = template.format_map (map)
            if not os.path.isabs (self.cmp_path) :
                self.cmp_path = os.path.join(workspace, self.cmp_path)

            self.cmp_path = os.path.realpath(self.cmp_path)
            self.cmp_name = os.path.split(self.cmp_path)[1]
            map[  prefix] = SimpleNamespace (path = self.cmp_path,
                                             name = self.cmp_name)
        except KeyError:
            self.cmp_path = None
    ...
    # --------------------------------------------------------------------------
    def complete_cfg_path (self, cfg_root, map, prefix, template) :
        try:
            self.cfg_path = template.format_map (map)
            if not os.path.isabs (self.cfg_path):
                self.cfg_path = os.path.join (cfg_root, self.cfg_path)


            self.cfg_path = os.path.realpath(self.cfg_path)
            self.cfg_dir,namext = os.path.split(self.cfg_path)
            self.cfg_name, self.cfg_ext = os.path.splitext (namext)
            if not self.cfg_ext:
                self.cfg_ext   = '.cfg'
                self.cfg_path += self.cfg_ext

            map[prefix] = SimpleNamespace (path = self.cfg_path,
                                           dir  = self.cfg_dir,
                                           name = self.cfg_name,
                                           ext  = self.cfg_ext)
        except KeyError:
...
class Targets:
    ...
    Args:
       ...
       filters:  A single list of strings containing the usual shell
                 wildcards
    ...
    def filter(targets, filters):
        ...
        for target in targets:
            for flt in flts:
                ...
                if match:
                    filtered.accepts.append(target)
                else:
                    filtered.rejects.append(target)
    ...
    def __init__(self, workspace, products):
        ...
        Args:
           ...
           products   :  The list of products. This comes from the user's
                         project file
        '''
        ...
        for product in products:
            ...
            for prd_target in prd_targets:
                ...
                for map in maps.maps:
                    ...
                    if target.cmp_name not in self.targets:
                        self.targets.append(target)
                        prd_target.cfg_path = target.cfg_path
                        prd_target.cfg_dir = target.cfg_dir
                        prd_target.cfg_name = target.cfg_name
                        prd_target.cfg_ext = target.cfg_dir
                        prd_target.cmp_name = target.cmp_name
                        prd_target.cfg_path = target.cfg_path
                    else:
                        print(
                            f"ERROR: duplicate component found {target.cmp_name}")
    ...
    def classify(workspace, products, targets, filters, configurations):
        '''
        Classifies potential configuration files as
           ...
           2. Missing  -- a configuration file that both is accepted by the
                          filters
              and is known to this project but is absent
           3. Cruft   --- A file in any of the directories with the file
                          extensions used by that directory when creating
        ...
        CAVEAT
        The Existing and Missing categories are hard concepts, i.e.
        ...
        is a very sure way
           1. The components in the workspace are examined to see if the json
              file contains a reference to the file

        If the corresponding component does not exist, this fails and uses a
        heuristic method.

        By definition, such files are not known to the project, whether it is
        a configuration file or not cannot be definitively determined. Consider 2
        files
        'file1.cfg' and 'file2.txt',  It is only by some convention that
        ...
        producer are unknowable.  There are 2 ways to address this

          1. Demand the user provide a template giving the naming convention.
             While formally correct this was deemed too burdensome and
             error-prone.
        ...
        Because of this, the user is encouraged to use a consistent naming
        ...
        # to avoid duplicates, which is possible with wildcards
        ...
        # Initialize the existing and cruft lists to empty and
        ...
        # -----------------------------------------------------------
        # This changes any target specific symbol translation to '*'
        ...
        # ---------------------------------------------------------------------
        # Process each unexpanded project target
        # ---------------------------------------
        for product in products:
            ...
            for prd_target in prd_targets:
                ...
                # directories to see if there is any cruft in there. This is
                # somewhat heuristic, no way to do it completely right, so
                # it is not critical.
                # ----------------------------------------------------------
                cfg_dir = prd_target.cfg_dir
                cfg_ext = prd_target.cfg_ext
                ...
                for file in files:
                    ...
                    if tgt:
                        # ------------------------------------------
                        # Target exists, so already accounted for
                        # ------------------------------------------
                        continue
...
if __name__ == "__main__":
    ...
    classes = Targets.classify(
        ...
        None)
    print(f"\n\nResults")
    for _ in classes.existing:
        print(f"present  = {_.cfg_path}")
    for _ in classes.missing:
        print(f"missing  = {_.cfg_path}")
    for _ in classes.cruft_lo:
        print(f"cruft_lo = {_.cfg_path}")
    for _ in classes.cruft_hi:
        print(f"cruft_hi = {_.cfg_path}")
