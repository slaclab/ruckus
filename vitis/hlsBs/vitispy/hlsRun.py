...
def merge (x, y) :
    if x :
        # Insure X is a list
        if len (x) == 1 : lst = x[0].split (',')
        else            : lst = x
    else                            : lst = []

    if y:
        if len (y) == 1 : lst += y[0].split (',')
        else            : lst += y
...
class Options :
    def __init__ (self,
        ...
        if  all is not None :
            ...
            if  package           : self.stages |= Run.Stage.Package
            if  implementation    : self.stages |= Run.Stage.Implementation

        if all is not None :
            if  csim == None : csim  = 'all'
            if cosim == None : cosim = 'all'
            if    ip == None : ip    = 'all'
            self. csim_set ( csim)
    ...
    def ip_set (ip_list) :
        ...
        if Options.is_in ('z', ip_list) : msk |= Run.Stage.Ip_Zip
        if Options.is_in ('d', ip_list) : msk |= Run.Stage.Ip_Dcp
    ...
    def action_msk (list, default_msk) :
        ...
        if all:
            ...
            if Options.is_in ('c', list) : msk |= Run.Msk.Clean
            if Options.is_in ('m', list) : msk |= Run.Msk.Make
            if Options.is_in ('r', list) : msk |= Run.Msk.Run
