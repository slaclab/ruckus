...
class Run:
    ...
    def announce(self, sep, caption, verbose=False):
        if verbose or self.options.verbose:
            if sep:
                self.printer.itemPlain('-' * self.printer.i, '')
            if caption:
                self.printer.itemPlain(caption, '')
        return True
    # --------------------------------------------------------------------------
    ...
    def report_vitis_norun(printer, verbose, line):
        ...
        if "ERROR" in line[0: 5]:
            suppress = False
        if "WARNING" in line[0: 7]:
            suppress = False
        if "make" in line[0: 4]:
            suppress = False
        if "Compiling" in line[0: 9]:
            suppress = False
        if "Generating" in line[0:10]:
            suppress = False
    ...
    def report_vitis_syn(printer, verbose, line):
        ...
        if "INFO" in line[0: 4]:
            suppress = True
        elif "WARNING: [RTGEN 206-101]" in line[0:24]:
            suppress = True
        elif "WARNING: [SYN 201-103]" in line[0:22]:
            suppress = True
        elif "**" in line[0: 6]:
            suppress = True
        elif "Resolution" in line[0:10]:
            suppress = True
        elif line.isspace():
            suppress = True
